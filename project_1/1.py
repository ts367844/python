...
def generate_pdf(rashi, text):

    filename = f"rashi_{date.today()}.pdf"

    styles = getSampleStyleSheet()

    story = []

    story.append(Paragraph("Daily Rashi Bhavishya", styles['Title']))
    story.append(Spacer(1,20))

    story.append(Paragraph(f"Rashi: {rashi.capitalize()}", styles['Heading2']))
...
def load_or_create_user_config():
    ...
    tob=input("Birth Time (HH:MM): ")
    place=input("Birth Place: ")

    config={"dob":dob,"tob":tob,"place":place}
